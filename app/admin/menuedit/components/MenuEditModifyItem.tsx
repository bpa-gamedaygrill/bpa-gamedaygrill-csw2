...
import { MenuEditInteractionDivInterface } from './MenuEditInteractionDiv';
...
interface MenuEditModifyItemInterface extends MenuEditInteractionDivInterface {
  modalActive: boolean;
  setModalActive: (value: boolean) => void;
}

const MenuEditModifyItem: React.FC<MenuEditModifyItemInterface> = ({ itemData, modalActive, setModalActive }) => {
  const closeBtnRef = useRef<HTMLButtonElement | null>(null);

  const [previewAvailable, setPreviewAvailable] = useState(false);
  const [closeModalActive, setCloseModalActive] = useState<boolean>(false);

  const [itemName, setItemName] = useState<string>(itemData.itemName);
  const [itemNameError, setItemNameError] = useState<boolean>(false);

  const [itemDesc, setItemDesc] = useState<string>(itemData.itemDesc);
  const [itemDescError, setItemDescError] = useState<boolean>(false);

  const [imageUrl, setImageUrl] = useState<string>(itemData.imageUrl);
  const [imageUrlError, setImageUrlError] = useState<boolean>(false);

  const [typeOfMeal, setTypeOfMeal] = useState<MenuCategoryFilterType>(itemData.itemType);

  const [itemPrice, setItemPrice] = useState<string>(itemData.itemPrice);
  ...
  const updateNewMenuItem = () => {
    ...
    setIsLoading(true)
    try {
      axios.post('/api/menu/updateitem', {
      ...
      <div className={`fixed w-full h-full top-0 left-0 z-[100] bg-black/20 ${modalActive ? "opacity-100 pointer-events-auto backdrop-blur-sm" : "opacity-0 pointer-events-none"} transition-all duration-500 flex items-center justify-center p-2`}>
        <div className="w-full h-full bg-white max-w-[450px] max-h-[500px] px-9 py-7 overflow-y-auto minimal-scrollbar rounded-lg">
          <div className="flex w-full items-center justify-between gap-3">
            <h1 className="font-semibold text-[1.35rem] tracking-tight opacity-80">Edit Menu Item</h1>
          ...
          <div className="flex flex-col mt-12 justify-start items-center w-full gap-8">
            ...
            <button className={`w-full py-2 px-4 bg-primary-red hover:bg-red-700 text-white font-semibold tracking-tight rounded-md ${isLoading && "!bg-red-700 cursor-auto opacity-80"}`} onClick={updateNewMenuItem} disabled={isLoading}>
...
export default MenuEditModifyItem;
